...
async function requireAdmin(ctx: { supabase: any; req: Request }) {
...
const getMyAccessibleCountries = publicProcedure.query(async ({ ctx }) => {
  ...
  const { data: profile } = await db(ctx.supabase)
    .from("profiles")
  ...
  const { data: extras } = await db(ctx.supabase)
    .from("user_countries")
    .select("country_id")
    .eq("user_id", user.id);
  ...
  if (accessibleIds.size === 0) {
    const { data: fallback } = await db(ctx.supabase)
      .from("countries")
      .select("id")
      .eq("code", "CH")
      .maybeSingle();
  ...
  if (ids.length > 0) {
    const { data } = await db(ctx.supabase)
      .from("countries")
      .select("id, code, name, flag_emoji, is_active")
      .in("id", ids)
      .order("name", { ascending: true });
