...
export function Header() {
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-[#e8e0d0] bg-[#fdfcf9]/96 shadow-[0_8px_28px_rgba(7,29,53,0.05)] backdrop-blur-xl">
      <div className="site-container flex min-h-[4.6rem] items-center justify-between gap-4 py-2.5 xl:min-h-24 xl:py-0">
        <Link
          href="/"
          onClick={closeMobileMenu}
          className="flex min-w-0 items-center gap-3 xl:w-[13.25rem] xl:flex-none"
        ...
        >
          <Image
            ...
            height={58}
            priority
            className="h-10 w-auto flex-none xl:h-14"
          />

          <span className="min-w-0 leading-tight">
            <span className="block truncate text-sm font-extrabold tracking-[-0.02em] text-[#0b2341] xl:text-[0.78rem]">
              Siebenten-Tags-Adventisten
            </span>
            <span className="mt-0.5 block truncate text-[0.68rem] font-bold text-[#496b3f] xl:text-[0.7rem]">
          ...
          {navigation.map((item) => {
            ...
            return (
              <Link
                key={item.href}
                href={item.href}
                className={`relative flex h-full min-w-0 items-center px-1.5 text-center text-[0.75rem] font-bold transition 2xl:px-2 2xl:text-[0.8rem] ${
                  active
                    ? "text-[#0b2341]"
        ...
        <div className="hidden flex-none items-center gap-2 xl:flex">
          <Link
            href="/app"
            className="inline-flex min-h-10 items-center gap-1.5 rounded-xl bg-[#d6a63f] px-3 py-2 text-[0.72rem] font-extrabold text-[#071d35] transition hover:bg-[#e3b956]"
          >
            <Smartphone size={15} />
            Church Connect
          </Link>
        ...
        <details ref={mobileMenuRef} className="group xl:hidden">
          <summary
            className="inline-flex h-11 w-11 cursor-pointer list-none items-center justify-center rounded-xl border border-[#e5dfd0] bg-white text-[#0b2341] shadow-sm [&::-webkit-details-marker]:hidden"
          ...
          <div
            ...
            className="absolute inset-x-0 top-full max-h-[calc(100dvh-4.6rem)] overflow-y-auto border-t border-[#e8e0d0] bg-[#fdfcf9] p-4 shadow-2xl"
          >
            <nav
